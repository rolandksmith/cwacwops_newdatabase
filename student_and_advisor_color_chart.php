function student_and_advisor_color_chart_func() {

/*	New Student and Advisor Color Chart V3

	Arrays
		combinedArray: 
			[0000|Monday,Thursday|English] => Array
				[Beginner Advisors] => 1
 	           	[Beginner Seats] => 6
		...
		studentArray: time | class days | language | level |  student array-class choice separated by commas
			example: 0000|Monday,Thursday|English|Beginner|A1ABC-1,A1BCD-2,A1CDE-3,...
		advisorArray: time | class days | language | level | advisors separated by commas
			example: 0000|Monday,Thursday|English|Beginner|AA8TA-1,K7OJL-2,WR7Q-1...
...
//	}
	...
	$inp_verbose				= "";
	$inp_mode					= "";
	$studentUpdateURL			= "$siteURL/cwa-student-and-advisor-color-chart/";	
	$notFilled					= array();
	$notIncluded			 	= array();
	$includedArray				= array();
...
// get the input information
	if (isset($_REQUEST)) {
		foreach($_REQUEST as $str_key => $str_value) {
			...
			if ($str_key 		== "notIncludedJson") {
				$notIncludedJson	 	= stripslashes($str_value);
//				$notIncludedJson	 	= filter_var($notIncludedJson,FILTER_UNSAFE_RAW);
			}
		}
	}


	function addCombinedArray($teachingTimeChunk,$teachingDays,$level,$language,$countType,$theCount,$callsign,$class) {
...
/*	where 
		...
		language is the class language (e.g., Catalan)
...
*/
		...
		$thisKey	= "$teachingTimeChunk|$teachingDays|$language";
		...
		if ($countType == 'Advisors') {
			...
			$advisorKey		= "$teachingTimeChunk|$teachingDays|$language|$level";
			...
			$specificKey	= "$level $countType";
			$combinedArray[$thisKey][$specificKey]++;
			$studentKey		= "$teachingTimeChunk|$teachingDays|$language|$level";
...
///// Pass 2 -- do the work
		...
		if ($wpw1_cwa_advisor === FALSE) {
			...
			if ($doDebug) {
				echo "ran $sql<br />and got $numARows rows from $advisorTableName table<br />";
			}
			if ($numARows > 0) {
				foreach ($wpw1_cwa_advisor as $advisorRow) {
					...
					if ($advisor_survey_score == '6' || $advisor_verify_response == 'R') {
						...
						if ($wpw1_cwa_advisorclass === FALSE) {
							...
							if ($doDebug) {
								echo "ran $sql<br />and got $numACRows rows from $advisorClassTableName<br />";
							}
							if ($numACRows > 0) {
								foreach ($wpw1_cwa_advisorclass as $advisorClassRow) {
									...
									if ($advisorClass_class_incomplete == 'Y') {
										...
										if ($advisorClass_call_sign == '') {
											$errorArray[]		= "advisorClass_call_sign missing for $advisorClass_advisor_callsign seq $advisorClass_sequence<br />";
										}
										...
										if ($goOn) {
											...
											$myResult		= addCombinedArray($thisTimes,$advisorClass_class_schedule_days_utc,$advisorClass_level,$advisorClass_language,'Advisors',$advisorClass_class_size,$advisor_call_sign,$advisorClass_sequence);
											if ($doDebug) {
												echo "&nbsp;&nbsp;&nbsp;&nbsp;did AddCombinedArray with $thisTimes,$advisorClass_class_schedule_days_utc,$advisorClass_level,$advisorClass_language,Advisors,$advisorClass_class_size,$advisor_call_sign,$advisorClass_sequence<br />";
											}
										}
									}
								}
		...
		if ($doProceed) {
			...
			foreach($combinedArray as $thisKey=>$thisValue) {
				...
				if ($myInt > 0) {
					$classOptionsArray[]	= "Beginner|$thisTime|$thisTeachingDays|$thisLanguage|$myInt";
				}
				$myInt						= $thisValue['Fundamental Advisors'];
				if ($myInt > 0) {
					$classOptionsArray[]	= "Fundamental|$thisTime|$thisTeachingDays|$thisLanguage|$myInt";
				}
				$myInt						= $thisValue['Intermediate Advisors'];
				if ($myInt > 0) {
					$classOptionsArray[]	= "Intermediate|$thisTime|$thisTeachingDays|$thisLanguage|$myInt";
				}
				$myInt						= $thisValue['Advanced Advisors'];
				if ($myInt > 0) {
					$classOptionsArray[]	= "Advanced|$thisTime|$thisTeachingDays|$thisLanguage|$myInt";
				}
			}
					...
					foreach ($wpw1_cwa_student as $studentRow) {
						...
						if ($doDebug) {
							echo "<br />Procesing $student_call_sign<br />
									student_class_language: $student_class_language<br />
						...
						if ($student_abandoned == 'Y') {
							$studentsDropped++;
							$studentsNotCounted++;
							$notIncluded[]			= "$student_call_sign- abandoned the signup process<br />";
						}
							...
							if ($beforeCatalog) {			// use catalog_options to chunk into 3-hour bocks
									...
									if ($convertResult === FALSE) {
										...
										if ($thisTimes == '') {			// no option returned
											...
											if (array_key_exists($thisTimes,$b4TimeGrouper)) {
												...
												$myResult		= addCombinedArray($displayTimes,$thisDays,$student_level,$thisLanguage,'Students',1,$student_call_sign,0);
												if ($doDebug) {
													echo "&nbsp;&nbsp;&nbsp;&nbsp;did AddCombinedArray with $displayTimes,$thisDays,$student_level,$thisLanguage,Students,1,$student_call_sign<br />";
												}
											} else {
												if ($doDebug) {
								...
								if ($student_first_class_choice == 'None' && ($student_second_class_choice == 'None') && ($student_third_class_choice == 'None')) {
									...
									if ($thisResult[0] == 'OK') {
										...
										$myResult		= addCombinedArray($thisTime,$thisDays,$student_level,$thisLanguage,'Students',1,$student_call_sign,0);
										if ($doDebug) {
											echo "&nbsp;&nbsp;&nbsp;&nbsp;did AddCombinedArray with $thisTime,$thisDays,$student_level,$thisLanguage,'Students',1,$student_call_sign,0<br />";
										}
								...
								if ($student_first_class_choice == 'None') {
									...
									if (count($myArray) == 2) {
										...
										$myStr					= "$displayTimes|$displayDays|$student_class_language";
										if ($doDebug) {
										...
										if (array_key_exists($myStr,$combinedArray)) {
											...
											if ($numberAdvisors > 0) {						
												// add info to the student portion of the combinedArray
												$myResult		= addCombinedArray($displayTimes,$displayDays,$student_level,$student_class_language,'Students',1,$student_call_sign,0);
												if ($doDebug) {
													echo "&nbsp;&nbsp;&nbsp;&nbsp;did AddCombinedArray with $displayTimes,$displayDays,$student_level,$student_class_language,Students,1,$student_call_sign<br />";
												}
												$gotClass				= TRUE;
...
// display the chart
			...
			foreach($combinedArray as $thisKey=>$thisValue) {
				...
				$thisLanguage			= $myArray[2];
				if ($doDebug) {
					echo "<br />Processing combinedArray $thisTime | $thisTeachingDays | $thisLanguage<br />";
				}
				if ($doDebug) {
				...
				if ($advisorCountBeg == 0) {
					...
					$begListKey			= "$thisTime|$thisTeachingDays|$thisLanguage|Beginner";
					$begListAdv			= $advisorArray[$begListKey];
					$begColumnTop		= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Beginner&inp_teaching_days=$thisTeachingDays&strpass=3&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$begListAdv' target='_blank'>$advisorCountBeg</a> | $classSizeCountBeg";
				...
				if ($advisorCountFun == 0) {
					...
					$funListKey			= "$thisTime|$thisTeachingDays|$thisLanguage|Fundamental";
					$funListAdv			= $advisorArray[$funListKey];
					$funColumnTop		= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Fundamental&inp_teaching_days=$thisTeachingDays&strpass=3&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$funListAdv' target='_blank'>$advisorCountFun</a> | $classSizeCountFun";
				...
				if ($advisorCountInt == 0) {
					...
					$intListKey			= "$thisTime|$thisTeachingDays|$thisLanguage|Intermediate";
					$intListAdv			= $advisorArray[$intListKey];
					$intColumnTop		= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Intermediate&inp_teaching_days=$thisTeachingDays&strpass=3&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$intListAdv' target='_blank'>$advisorCountInt</a> | $classSizeCountInt";
				...
				if ($advisorCountAdv == 0) {
					...
					$advListKey			= "$thisTime|$thisTeachingDays|$thisLanguage|Advanced";
					$advListAdv			= $advisorArray[$advListKey];
					$advColumnTop		= "<a href='$theURL?inp_time=$thisTime$thisLanguage=$thisLanguage&inp_level=Advanced&inp_teaching_days=$thisTeachingDays&strpass=3&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$advListAdv' target='_blank'>$advisorCountAdv</a> | $classSizeCountAdv";
				...
				if ($studentCountBeg == 0 && $advisorCountBeg == 0) {
					...
					if ($studentCountBeg != 0) {
						...
						$begColumnBottom	= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Beginner&inp_teaching_days=$thisTeachingDays&strpass=4&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$begListStu' target='_blank'>$studentCountBeg</a> | $excessBeg";
					} else {
				...
				if ($studentCountFun == 0 && $advisorCountFun == 0) {
					...
					if ($studentCountFun != 0) {
						...
						$funColumnBottom	= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Fundamental&inp_teaching_days=$thisTeachingDays&strpass=4&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$funListStu' target='_blank'>$studentCountFun</a> | $excessFun";
					} else {
				...
				if ($studentCountInt == 0 && $advisorCountInt == 0) {
					...
					if ($studentCountInt != 0) {
						...
						$intColumnBottom	= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage,inp_level=Intermediate&inp_teaching_days=$thisTeachingDays&strpass=4&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$intListStu' target='_blank'>$studentCountInt</a> | $excessInt";
					} else {
				...
				if ($studentCountAdv == 0 && $advisorCountAdv == 0) {
					...
					if ($studentCountAdv != 0) {
						...
						$advColumnBottom	= "<a href='$theURL?inp_time=$thisTime&thisLanguage=$thisLanguage&inp_level=Advanced&inp_teaching_days=$thisTeachingDays&strpass=4&inp_semester=$inp_semester&inp_verbose=$inp_verbose&inp_mode=$inp_mode&inp_list=$advListStu' target='_blank'>$studentCountAdv</a> | $excessAdv";
					} else {
			...
			$notIncludedJson	= json_encode($notIncluded);
			$content			.= "<p>$myStr<br />
									<b>Counts:</b><br />
									<b>Raw Data:</b><br />
									...
									$studentsNotCounted: <a href='$theURL/?notIncludedJson=$notIncludedJson&strpass=10' 
										target='_blank'>Students not included in chart due to errors</a><br /><br />";
			...
			$content			.= "<br /><b>Current Advisors Planned classes</b>
			...
			foreach($classOptionsArray as $myValue) {
				...
				$advisorList	= '';
				$advisorArrayKey = "$thisTime|$thisDays|$thisLanguage|$thisLevel";
...
// $doDebug = TRUE;		
		...
		foreach ($myArray as $myValue) {
			...
			$sql					= "select * from $advisorClassTableName 
										...
										where advisorclass_call_sign='$tempAdvisor' 
										and advisorclass_semester='$inp_semester' 
										and advisorclass_sequence=$tempClass";
			$wpw1_cwa_advisorclass				= $wpdb->get_results($sql);
			if ($wpw1_cwa_advisorclass !== FALSE) {
				$numACRows									= $wpdb->num_rows;
				if ($numACRows > 0) {
					foreach ($wpw1_cwa_advisorclass as $advisorClassRow) {
						...
						if ($doDebug) {
							echo "<br />Got advisor $advisorClass_call_sign<br />
			...
			if ($wpw1_cwa_student !== FALSE) {
				...
				if ($numSRows > 0) {
					foreach ($wpw1_cwa_student as $studentRow) {
						...
						$content	.= "<tr><td><a href='$updateStudentURL?strpass=2&request_type=callsign&request_info=$studentCallSign&inp_depth=one&doDebug=$doDebug&testMode=$testMode' target='_blank'>$studentCallSign</a></td>
...
////////// Pass 10 - show not included array
	} elseif ("10" == $strPass) {
		if ($doDebug) {
			echo "<br />at pass10<br />";
		}
//		if ($doDebug) {
			echo "notIncludedJson: $notIncludedJson<br />";
//		}
		
		$content				.= "<h3>$jobname</h3>
									<h4>Students Not Included</h4>";
		$myArray				= json_decode($notIncludedJson,TRUE);
		if ($doDebug) {
			echo "notIncludedJson: $notIncludedJson<br />";
		}
		foreach($myArray as $thisString) {
			$stringArray		= explode("-",$thisString);
			$student_call_sign 	= $stringArray[0];
			$strReason			= $stringArray[1];
			$content			.= "<a href='$updateStudentURL?request_type=callsign&request_info=$student_call_sign&doDebug=$doDebug&testMode=$testMode' target='_blank'>$student_call_sign</a>$strReason";
		}
	}
...
add_shortcode ('student_and_advisor_color_chart', 'student_and_advisor_color_chart_func');
