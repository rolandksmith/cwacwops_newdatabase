function update_advisor_database_tables_func() {

// Upgrade Advisor Database Tables
...
'Zimbabwe,263,ZW / ZWE');
	...
	foreach($phoneCodeArray as $thisValue) {
		$myArray			= explode(",",$thisValue);
		$countryName		= $myArray[0];
		$phoneCode			= $myArray[1];
		$myStr				= $myArray[2];
		$codeArray			= explode("/",$myStr);
		$countryCode		= trim($codeArray[0]);
		$countryCode3		= trim($codeArray[1]);
		
		$result				=	$wpdb->insert(')
	}

}
add_shortcode ('update_advisor_database_tables', 'update_advisor_database_tables_func');