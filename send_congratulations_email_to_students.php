...
///// Pass 2 -- do the work
		...
		$sql					= "select * from $studentTableName 
									...
									where student_semester='$thisSemester' 
					...
					if ($student_promotable != 'W') {
						...
						if ($student_promotable == 'N') {
							...
							$sql				= "select * from $studentTableName 
												   ...
												   limit 1";
							$wpw1_cwa_student1	= $wpdb->get_results($sql);
							if ($wpw1_cwa_student1 === FALSE) {
								handleWPDBError($jobname,$doDebug);
							} else {
								$numberSRecords	= $wpdb->num_rows;
								if ($doDebug) {
									echo "ran $sql<br />and retrieved $numberSRecords rows from $studentTableName table<br />";
								}
								...
								if (in_array($student_level,$bbiArray)) {			// bbi student
									if ($numberSRecords == 0) {				// not promotable, not in a class
										...
										$myArray				= array();
										foreach ($wpw1_cwa_student1 as $studentRow) {
											$prev_student_level  						= $studentRow->student_level;
											$prev_student_semester						= $studentRow->student_semester;
						...
						if ($student_promotable == 'P') {
							...
							$sql				= "select * from $studentTableName 
												   ...
												   order by student_date_created 
												   limit 1";
							$wpw1_cwa_student2	= $wpdb->get_results($sql);
							if ($wpw1_cwa_student2 === FALSE) {
								handleWPDBError($jobname,$doDebug);
							} else {
								$numSRecords	= $wpdb->num_rows;
								if ($doDebug) {
									echo "ran $sql<br />and retrieved $numSRecords rows from $studentTableName table<br />";
								}
								if (in_array($student_level,$bbiArray)) {			// bbi student
									if ($numSRecords == 0) {				// not promotable, not in a class
										...
										$myArray				= array();
										foreach ($wpw1_cwa_student2 as $studentRow) {
											$prev_student_level  	= $studentRow->student_level;
											$prev_student_semester	= $studentRow->student_semester;
											$myArray[]			= "$prev_student_level,$prev_student_semester";
										}
