...
address by looking into the advisor, student, and sent email tables.</p>
...
///// Pass 2 -- do the work
		...
		if ($doDebug) {
			echo "At pass 2 with callsign $inp_callsign or email $inp_email<br />";
		}
		$thisReqStr					= '';
					...
					if ($result === FALSE) {
						...
						if ($numRows > 0) {
							foreach($result as $resultRows) {
								...
								$thisReqStr				= "$inp_callsign at $thisEmail";
								if ($doDebug) {
