				...
				foreach ($wpw1_cwa_student as $studentRow) {
					...
					if ($currentSemester == 'Not in Session' && $student_semester == $nextSemester) {
						if ($student_response == '') {
							...
							$issue30Array[]		= "<a href='$updateStudentURL?strpass=2&request_type=callsign&request_info=$student_call_sign&request_table=$studentTableName' target='_blank'>$student_call_sign</a>";
							$issue30++;
							$hasError			= TRUE;
							if ($doDebug) {
					...
					if ($student_student_status != '') {
						if (!in_array($student_student_status,$validStatuses)) {
							$errorReport	.= "<b>Issue 04</b> Student has an invalid Student Status of |$student_student_status|<br />";
					...
					if (in_array($student_semester,$futureSemesters)) {
						...
						if ($student_student_status != '') {
							$errorReport	.= "<b>Issue 15</b> Student should not have a Student Status of |$student_student_status|<br />";
							$issue15++;
							$hasError			= TRUE;
